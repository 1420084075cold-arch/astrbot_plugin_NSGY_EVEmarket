...
class MyPlugin(Star):
    ...
    def get_type_id_by_name(name: str) -> Optional[int]:
        ...
        """
        # 1. 尝试 fuzzwork（英文名）
        type_id = get_type_id_by_name_fuzzwork(name)
        if type_id:
            return type_id

        # 2. 失败则尝试 ESI 的多语言名称搜索（支持中文）
        type_id = get_type_id_by_name_esi(name)
        return type_id
    ...
    async def jita(self, event: AstrMessageEvent, content_message: str):
        ...
        if min_sell is None and max_buy is None:
            ...
            yield event.plain_result("Jita 最低卖价:", min_sell if min_sell is not None else "无")
            yield event.plain_result("Jita 最高买价:", max_buy if max_buy is not None else "无")
        # yield event.plain_result(f"{content_message}") 
