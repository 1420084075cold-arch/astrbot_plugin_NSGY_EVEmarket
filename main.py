...
class MyPlugin(Star):
    ...
    def check_new_killmails(self):
        ...
        try:
            resp = requests.get(url, params=params, timeout=15, headers={"User-Agent": "AstrBot-EVE-Market-Plugin/1.0"})
            if resp.status_code == 200:
        ...
        # 过滤匹配订阅的击杀邮件
        new_mails_by_group = {}  # {group_id: [killmail]}
        ...
        for killmail in all_killmails:
            ...
            # 检查匹配哪些群组
            matched_groups = self.get_matching_groups(killmail)
            ...
            if matched_groups:
                ...
                for group_id in matched_groups:
                    if group_id not in new_mails_by_group:
                        new_mails_by_group[group_id] = []
                    new_mails_by_group[group_id].append(killmail)
        ...
        # 发送通知到各个群组
        for group_id, mails in new_mails_by_group.items():
            for killmail in mails:
                self.send_killmail_notification(group_id, killmail)
    
    def get_matching_groups(self, killmail: dict) -> Set[str]:
        """检查击杀邮件匹配哪些群组的订阅，返回群组ID列表"""
        matched_groups = set()
        ...
        for attacker in attackers:
            if attacker.get("character_id"):
                attacker_chars.add(attacker.get("character_id"))
        ...
        for group_id, subs in self.subscriptions.items():
            ...
            # 检查价格阈值订阅
            min_value_key = "min_value:" + str(self.MIN_KILL_VALUE)
            if min_value_key in subs and kill_value >= self.MIN_KILL_VALUE:
                group_matched = True
                logger.info(f"击杀价值 {kill_value:,.2f} ISK 达到 {self.MIN_KILL_VALUE:,.2f} ISK 阈值")
            
            # 检查具体订阅
            for sub in subs:
                if sub.startswith("min_value:"):
                    continue  # 已经处理过
                ...
                if sub_type == "corp":
                    ...
                    # 订阅玩家：被击杀方或击杀方
                    if victim_char_id == sub_id or sub_id in attacker_chars:
                        group_matched = True
                        break
                
                elif sub_type == "player_kill":
                    # 订阅玩家的击杀
                    if sub_id in attacker_chars:
                        group_matched = True
                        break
    ...
    def send_killmail_notification(self, group_id: str, killmail: dict):
        """发送击杀邮件通知到指定群组"""
        kill_id = killmail.get("killmail_id")
        victim = killmail.get("victim", {})
        ...
        victim_sec_status = victim.get("security_status", 0)
        ...
        for attacker in attackers:
            ...
            if name and name != "Unknown":
                ...
                attacker_infos.append(f"{name} ({ship}) - {damage_pct:.1f}%")
        
        attacker_str = "\n".join(attacker_infos) if attacker_infos else "Unknown"
        
        # 判断是否是高安/低安/00
        solar_system = killmail.get("solar_system", {})
        system_sec = solar_system.get("security_status", 0)
        if system_sec >= 0.5:
        security_status_msg = f"🛡️ {system_sec:.1f} (高安)"
        elif system_sec > 0.0:
            security_status_msg = f"⚠️ {system_sec:.1f} (低安)"
        else:
            security_status_msg = f"💀 {system_sec:.1f} (00)"
        ...
        if victim_alliance:
            message_parts.append(f"🌟 **联盟**: {victim_alliance}")
        
        message_parts.extend([
            f"🚀 **舰船**: {victim_ship}",
            f"💰 **价值**: {total_value:,.2f} ISK",
            f"💎 **掉落价值**: {dropped_value:,.2f} ISK",
            f"🔨 **摧毁价值**: {destroyed_value:,.2f} ISK",
            f"⭐ **安全等级**: {security_status_msg}",
            "",
            f"🎯 **击杀者 (伤害占比)**:",
            f"{attacker_str}",
            "",
            f"🔗 **详情**: {self.ZKILL_REDIRECT}/{kill_id}/"
        ])
        
        # 如果是高价值击杀，添加星号标记
        if total_value >= 10_000_000_000:
            message_parts.insert(0, "🌟 **✨高价值击杀！✨** 🌟")
        
        message = "\n".join(message_parts)
        
        # 发送消息（这里需要根据 AstrBot 的 API 调整）
        # 由于 AstrBot 的消息发送机制，这里记录日志并尝试发送
        logger.info(f"推送击杀邮件到群组 {group_id}: {victim_name} 在 {victim_ship} 被击杀，价值 {total_value:,.2f} ISK")
        
        # 实际发送消息的代码需要根据 AstrBot 的 API 来写
        # 这里提供一种可能的实现方式，可能需要根据你的 AstrBot 版本调整
        try:
            # 假设有 self.send_message_to_group 方法
            # 如果没有，可以通过 event 对象的回调发送
            # 暂时使用 logger 记录，实际使用时需要替换为真实的消息发送代码
            # await self.context.send_group_message(int(group_id), message)
            pass
        except Exception as e:
    ...
    async def subscribe(self, event: AstrMessageEvent):
        ...
        用法:
            ...
            .sub alliance [联盟ID]    # 订阅联盟（被击杀或击杀时通知）
            .sub player [角色ID]      # 订阅玩家（被击杀或击杀时通知）
            .sub player_kill [角色ID] # 订阅玩家的击杀
            .sub player_loss [角色ID] # 订阅玩家的被击杀
            .sub min_value            # 订阅10E以上价值的击杀
            .sub list                 # 查看当前订阅
        ...
        if len(parts) < 2:
            yield event.plain_result("用法:\n.sub corp/alliance/player [ID]\n.sub player_kill/player_loss [ID]\n.sub min_value\n.sub list/clear")
            return
        ...
        if command == "list":
            ...
            if not subs:
                ...
                result_parts = ["📋 **当前订阅**:\n"]
                for sub in subs:
                    if sub.startswith("corp:"):
                        result_parts.append(f"  - 🏢 军团 ID: {sub.split(':')[1]}")
                    elif sub.startswith("alliance:"):
                        result_parts.append(f"  - 🌟 联盟 ID: {sub.split(':')[1]}")
                    elif sub.startswith("player:"):
                        result_parts.append(f"  - 👤 玩家 (击杀+被击杀) ID: {sub.split(':')[1]}")
                    elif sub.startswith("player_kill:"):
                        result_parts.append(f"  - 🔫 玩家击杀 ID: {sub.split(':')[1]}")
                    elif sub.startswith("player_loss:"):
                        result_parts.append(f"  - 💀 玩家被击杀 ID: {sub.split(':')[1]}")
                    elif sub.startswith("min_value"):
                        result_parts.append(f"  - 💰 价值阈值: {self.MIN_KILL_VALUE:,.0f} ISK (10E+)")
                yield event.plain_result("\n".join(result_parts))
            return
        ...
        if command == "min_value":
            sub_key = f"min_value:{self.MIN_KILL_VALUE}"
            self.subscriptions[group_id].add(sub_key)
            yield event.plain_result(f"✅ 已订阅价值 {self.MIN_KILL_VALUE:,.0f} ISK (10E) 以上的高价值击杀")
            return
        ...
        if not entity_id.isdigit():
            yield event.plain_result("ID 必须是数字，使用 .search [名称] 搜索获取ID")
            return
    ...
    async def unsubscribe(self, event: AstrMessageEvent):
        ...
        用法:
            ...
            .unsub player [角色ID]       # 取消订阅玩家
            .unsub player_kill [角色ID]  # 取消订阅玩家击杀
            .unsub player_loss [角色ID]  # 取消订阅玩家被击杀
            .unsub min_value             # 取消订阅价值阈值
            .unsub [type]                # 不写ID则取消该类型所有订阅
        """
        content = event.message_str.strip()
        parts = content.split()
        
        if len(parts) < 2:
            yield event.plain_result("用法: .unsub corp/alliance/player/player_kill/player_loss/min_value [ID]")
            return
        ...
        if command == "min_value":
            ...
            if sub_key in self.subscriptions[group_id]:
                ...
                yield event.plain_result("✅ 已取消订阅高价值击杀")
            else:
        ...
        # 没有ID的情况 - 清除该类型所有订阅
        if len(parts) < 3:
            type_to_remove = command
            if type_to_remove in ["corp", "alliance", "player", "player_kill", "player_loss"]:
                to_remove = [s for s in self.subscriptions[group_id] if s.startswith(f"{type_to_remove}:")]
                for s in to_remove:
                    self.subscriptions[group_id].remove(s)
                yield event.plain_result(f"✅ 已移除所有 {type_to_remove} 类型订阅")
            else:
                yield event.plain_result(f"未知的订阅类型: {command}")
            return
        ...
        if sub_key in self.subscriptions[group_id]:
            ...
            yield event.plain_result(f"❌ 未订阅 {command} ID: {entity_id}")
    ...
    async def search_entity(self, event: AstrMessageEvent, name: str = ""):
        ...
        try:
            ...
            if characters:
                result_parts.append("👤 **角色**:")
                for char in characters[:5]:
                    result_parts.append(f"  - {char.get('name')} (ID: {char.get('id')})")
                    result_parts.append(f"    订阅命令: .sub player {char.get('id')}")
                    result_parts.append(f"    订阅击杀: .sub player_kill {char.get('id')}")
                    result_parts.append(f"    订阅被击杀: .sub player_loss {char.get('id')}")
            ...
            if corporations:
                ...
                for corp in corporations[:5]:
                    ...
                    result_parts.append(f"    订阅命令: .sub corp {corp.get('id')}")
            ...
            if alliances:
                ...
                for ally in alliances[:5]:
                    ...
                    result_parts.append(f"    订阅命令: .sub alliance {ally.get('id')}")
            
            if result_parts:
                result_parts.append("\n💡 **提示**: 使用对应的 .sub 命令订阅后，将自动推送击杀邮件")
                result_parts.append("💰 高价值击杀: .sub min_value 订阅10E以上击杀")
                yield event.plain_result("\n".join(result_parts))
            else:
                yield event.plain_result(f"未找到与「{name}」相关的角色、军团或联盟")
    ...
    # 请将之前回复中的市场查询方法复制到这里
新增订阅类型说明
1. .sub min_value - 订阅高价值击杀
订阅价值 10E ISK (10,000,000,000) 以上的击杀邮件

2. .sub player [角色ID] - 订阅玩家所有活动
当该玩家击杀或被击杀时都会通知

3. .sub player_kill [角色ID] - 订阅玩家击杀
只有当该玩家击杀他人时才通知

4. .sub player_loss [角色ID] - 订阅玩家被击杀
只有当该玩家被击杀时才通知
...
# 订阅玩家的所有活动
.sub player 1234567890

# 只订阅玩家的击杀
.sub player_kill 1234567890

# 只订阅玩家的被击杀
.sub player_loss 1234567890

# 订阅军团
.sub corp 98654321

# 订阅联盟
.sub alliance 1234567890

# 查看当前订阅
.sub list

# 取消订阅
.unsub player 1234567890
.unsub min_value
.unsub player_kill 1234567890
通知示例
高价值击杀通知（会额外显示星星标记）
text
🌟 ✨高价值击杀！✨ 🌟
...
...
配置文件（可选）
如果你想修改价值阈值（比如改成50E或100E），可以修改类常量：

python
MIN_KILL_VALUE = 10_000_000_000  # 10E，可以改成 50_000_000_000 或 100_000_000_000
或者添加一个命令来动态修改：

python
@filter.command(".minvalue")
async def set_min_value(self, event: AstrMessageEvent, value: str):
    """设置高价值击杀阈值（单位：E，例如 .minvalue 50 表示50E）"""
    try:
        value_e = float(value)
        self.MIN_KILL_VALUE = int(value_e * 1_000_000_000)
        yield event.plain_result(f"已设置高价值击杀阈值为 {value_e}E ISK ({self.MIN_KILL_VALUE:,} ISK)")
    except:
        yield event.plain_result("格式错误，例如: .minvalue 50 表示50E")
需要我帮你添加更多功能吗？比如：
...
能截取图片吗
可以的！EVE ESI 和 zKillboard 都提供了击杀邮件的截图功能。我来帮你添加击杀邮件截图功能。
...
class MyPlugin(Star):
    ...
    MIN_KILL_VALUE = 10_000_000_000  # 10E ISK
    
    # 截图相关配置
    SCREENSHOT_ENABLED = True  # 是否启用截图功能
    SCREENSHOT_API = "https://image.eveonline.com"  # EVE 官方图片API
    # 或使用 zKillboard 的截图: https://zkillboard.com/kill/{kill_id}/screenshot/
    
    # 存储订阅信息
    subscriptions: Dict[str, Set[str]] = {}
    killmail_history: Set[str] = set()
    ...
    def get_killmail_screenshot(self, kill_id: int) -> Optional[bytes]:
        """
        获取击杀邮件的游戏内截图
        
        方法1: 使用 zKillboard 的截图功能
        方法2: 使用 EVE 官方 API 拼接舰船图片
        
        返回图片的二进制数据
        """
        try:
            # 方法1: zKillboard 截图（最直观）
            # zKillboard 会自动生成击杀邮件的图片版本
            ...
            # 尝试获取图片版本
            image_url = f"https://zkillboard.com/kill/{kill_id}/screenshot/"
            response = requests.get(image_url, headers=headers, timeout=15)
            
            if response.status_code == 200 and response.content:
                logger.info(f"成功获取击杀 {kill_id} 的截图")
                return response.content
            
            # 如果截图不存在，尝试使用 zKillboard 的渲染图片
            render_url = f"https://zkillboard.com/kill/{kill_id}/render/"
            response = requests.get(render_url, headers=headers, timeout=15)
            
            if response.status_code == 200 and response.content:
                logger.info(f"成功获取击杀 {kill_id} 的渲染图")
                return response.content
    ...
    def get_ship_image(self, ship_type_name: str) -> Optional[bytes]:
        """
        获取舰船图片（通过 EVE 官方图片服务器）
        """
        try:
            # 首先需要根据舰船名称获取 type_id
            type_id = self.get_type_id_by_name_fuzzwork(ship_type_name)
            if not type_id:
                return None
            
            # EVE 官方图片API
            # 256x256 尺寸的舰船图标
            image_url = f"https://images.eveonline.com/types/{type_id}/256"
    ...
    def get_character_portrait(self, character_id: int, size: int = 64) -> Optional[bytes]:
        ...
        """
        try:
            image_url = f"https://images.eveonline.com/Character/{character_id}_{size}"
            response = requests.get(image_url, timeout=10)
            if response.status_code == 200:
                return response.content
            return None
        except Exception as e:
            logger.error(f"获取角色肖像失败: {e}")
            return None
    ...
    def get_corporation_logo(self, corporation_id: int, size: int = 64) -> Optional[bytes]:
        ...
        """
        try:
            image_url = f"https://images.eveonline.com/Corporation/{corporation_id}_{size}"
            response = requests.get(image_url, timeout=10)
            if response.status_code == 200:
                return response.content
            return None
        except Exception as e:
            logger.error(f"获取军团标志失败: {e}")
            return None
    
    def get_alliance_logo(self, alliance_id: int, size: int = 64) -> Optional[bytes]:
        """
        获取联盟标志
        """
        try:
            image_url = f"https://images.eveonline.com/Alliance/{alliance_id}_{size}"
            response = requests.get(image_url, timeout=10)
            if response.status_code == 200:
                return response.content
            return None
        except Exception as e:
            logger.error(f"获取联盟标志失败: {e}")
            return None
    
    def get_killmail_map_image(self, killmail: dict) -> Optional[bytes]:
        """
        获取击杀地点的星图截图（通过 Dotlan 或其他服务）
        """
        try:
            solar_system = killmail.get("solar_system", {})
            system_name = solar_system.get("name", "")
            if not system_name:
                return None
            
            # 使用 Dotlan 的星图（需要 URL 编码）
            import urllib.parse
            encoded_name = urllib.parse.quote(system_name)
            
            # Dotlan 星图（可能不稳定，作为备选）
            # map_url = f"https://evemaps.dotlan.net/map/{encoded_name}/small"
            
            # 使用 zKillboard 的地图截图
            kill_id = killmail.get("killmail_id")
            map_url = f"https://zkillboard.com/kill/{kill_id}/map/"
    ...
    def send_killmail_notification(self, group_id: str, killmail: dict):
        ...
        # 获取击杀者信息
        attackers = killmail.get("attackers", [])[:5]
        main_attacker = attackers[0] if attackers else {}
        main_attacker_name = main_attacker.get("character_name", "Unknown")
        main_attacker_ship = main_attacker.get("ship_type_name", "Unknown")
        ...
        message_parts.extend([
            ...
            f"💰 **价值**: {total_value:,.2f} ISK",
            f"💎 **掉落价值**: {dropped_value:,.2f} ISK",
            f"🎯 **击杀者**: {main_attacker_name} ({main_attacker_ship})",
            "",
            f"🔗 **详情**: {self.ZKILL_REDIRECT}/{kill_id}/"
        ])
        
        if total_value >= self.MIN_KILL_VALUE:
            message_parts.insert(0, "🌟 **✨高价值击杀！✨** 🌟")
        
        text_message = "\n".join(message_parts)
        
        # 发送文本消息
        # yield event.plain_result(text_message)  # 这里需要根据实际发送方式调整
        ...
        if self.SCREENSHOT_ENABLED:
            ...
            if screenshot:
                # 如果有截图，发送图片
                # 注意：这里需要根据 AstrBot 的图片发送API调整
                # 假设有 send_image 方法
                try:
                    # 将图片转换为 base64 或直接发送
                    import base64
                    image_base64 = base64.b64encode(screenshot).decode('utf-8')
                    # 发送图片消息的代码（根据实际 AstrBot API 调整）
                    # await self.context.send_image(group_id, image_base64)
                    logger.info(f"已获取截图，大小: {len(screenshot)} bytes")
                except Exception as e:
    ...
    async def get_screenshot(self, event: AstrMessageEvent, kill_id: str = ""):
        """手动获取击杀邮件截图
        ...
        """
        if not kill_id:
            yield event.plain_result("请输入击杀ID，例如: .screenshot 12345678")
            return
        
        try:
            kill_id = int(kill_id.strip())
        except ValueError:
        ...
        if screenshot:
            ...
            import base64
            image_base64 = base64.b64encode(screenshot).decode('utf-8')
            # await self.context.send_image(event.group_id, image_base64)
            yield event.plain_result(f"✅ 截图获取成功！大小: {len(screenshot)} bytes")
            # 实际发送图片的代码需要根据 AstrBot 的 API 来写
        else:
            yield event.plain_result(f"❌ 无法获取击杀 {kill_id} 的截图")

    @filter.command(".shipimg")
    async def get_ship_image_cmd(self, event: AstrMessageEvent, ship_name: str = ""):
        """获取舰船图片
        ...
        """
        if not ship_name:
            yield event.plain_result("请输入舰船名称，例如: .shipimg Rifter")
            return
        
        yield event.plain_result(f"正在获取 {ship_name} 的图片...")
        
        image = self.get_ship_image(ship_name)
        
        if image:
            import base64
            image_base64 = base64.b64encode(image).decode('utf-8')
            # await self.context.send_image(event.group_id, image_base64)
            yield event.plain_result(f"✅ 图片获取成功！")
        else:
    ...
    async def subscribe(self, event: AstrMessageEvent):
        ...
        if command == "corp":
            self.subscriptions[group_id].add(f"corp:{entity_id}")
            yield event.plain_result(f"✅ 已订阅军团 ID: {entity_id}")
        elif command == "alliance":
            self.subscriptions[group_id].add(f"alliance:{entity_id}")
            yield event.plain_result(f"✅ 已订阅联盟 ID: {entity_id}")
        elif command == "player":
            self.subscriptions[group_id].add(f"player:{entity_id}")
            yield event.plain_result(f"✅ 已订阅玩家 ID: {entity_id}")
        elif command == "player_kill":
            self.subscriptions[group_id].add(f"player_kill:{entity_id}")
            yield event.plain_result(f"✅ 已订阅玩家击杀 ID: {entity_id}")
        elif command == "player_loss":
            self.subscriptions[group_id].add(f"player_loss:{entity_id}")
            yield event.plain_result(f"✅ 已订阅玩家被击杀 ID: {entity_id}")
        else:
            yield event.plain_result(f"❌ 未知的订阅类型: {command}")
    ...
    async def search_entity(self, event: AstrMessageEvent, name: str = ""):
        ...
        try:
            ...
            if result_parts:
                result_parts.append("\n💡 使用 .sub corp/alliance/player [ID] 订阅")
                result_parts.append("💰 高价值击杀: .sub min_value 订阅10E以上击杀")
                yield event.plain_result("\n".join(result_parts))
            else:
                yield event.plain_result(f"未找到与「{name}」相关的角色、军团或联盟")
